...
//Aloca a mem�ria necess�ria para armazenar uma matriz
{
	tipo_dados *ptr;
...
void escreverMatriz(tipo_dados *M, int nl, int nc)
{
	...
	for (i=0;i<nl;i++)
		for (j=0;j<nc;j++)
			printf("\nElemento da matriz na posi��o %d,%d = %d",i+1,j+1,*(M++));
}
...
void main()
{
	...
	do
	{
		printf("N�mero de colunas da matriz A: ");
		scanf("%d",&n);
	}while(n<=0);
    ...
    // Matriz B  (n�mero de linhas de B = numero colunas de A) - exercicio 2
    do
	{
		printf("\nN�mero de colunas da matriz B: ");
		scanf("%d",&p);
	}while(n<=0);
